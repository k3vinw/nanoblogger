...
# check if category feeds are enabled
if [ "$CATEGORY_FEEDS" = 1 ] || [ "$ATOM_CATFEEDS" = 1 ] || [ "$RSS2_CATFEEDS" = 1 ] || [ "$RSS_CATFEEDS" = 1 ]; then
	catfeeds_enabled=true
fi
...
if [ ! -z "$NB_AtomVer" ]; then
	...
	if [ "$MKPAGE_TEMPLATE" = "$NB_TEMPLATE_DIR/$CATEGORY_TEMPLATE" ] && [ "$catfeeds_enabled" = true ]; then
